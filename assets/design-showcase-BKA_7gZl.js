...
`;var d={showcase:document.querySelector(`.showcase`),playStatus:document.querySelector(`#playStatus`),chapterCount:document.querySelector(`#chapterCount`),stageMeta:document.querySelector(`#stageMeta`),stageFrame:document.querySelector(`#stageFrame`),stageEyebrow:document.querySelector(`#stageEyebrow`),stageTitle:document.querySelector(`#stageTitle`),stageDescription:document.querySelector(`#stageDescription`),previous:document.querySelector(`#previousChapter`),toggle:document.querySelector(`#togglePlay`),next:document.querySelector(`#nextChapter`),progress:document.querySelector(`#chapterProgress`),rail:document.querySelector(`#chapterRail`)},f=0,p=!1,m=!1,h,g,_,v=8e3;function y(){let e=[`brand`,`technology`];d.rail.innerHTML=`
    ...
    <div class="preview-host preview-host--${e.type}">
      <div class="preview-frame" style="--frame-radius: ${e.type===`mobile`?`42px`:e.type===`pad`?`30px`:`12px`}">
        <div class="preview-surface">
          <iframe src="${r}" title="${e.eyebrow} ${e.title}" allow="autoplay; fullscreen"></iframe>
        </div>
      </div>
  ...
  `;let i=d.stageFrame.querySelector(`.preview-host`),a=d.stageFrame.querySelector(`.preview-frame`),o=d.stageFrame.querySelector(`.preview-surface`),s=d.stageFrame.querySelector(`iframe`);b(e,i,a,o),s.addEventListener(`load`,()=>{s.isConnected&&(d.stageFrame.classList.add(`is-loaded`),d.stageFrame.querySelector(`.frame-loader`)?.remove())},{once:!0})}function S(){d.playStatus.classList.toggle(`is-live`,p),d.playStatus.querySelector(`b`).textContent=p?`AUTO PLAY`:`MANUAL`,d.chapterCount.textContent=`${String(f+1).padStart(2,`0`)} / ${String(i.length).padStart(2,`0`)}`,d.toggle.innerHTML=o(p?`pause`:`play`,18,!0),d.toggle.setAttribute(`aria-label`,p?`暂停自动播放`:`继续自动播放`)}function C(){window.clearTimeout(h),d.progress.classList.remove(`is-running`),d.progress.style.setProperty(`--chapter-duration`,`${v}ms`),d.progress.offsetWidth,!(!p||!m)&&(d.progress.classList.add(`is-running`),h=window.setTimeout(()=>{if(f===i.length-1){p=!1,S();return}f+=1,w()},v))}function w(e){let t=i[f];d.stageMeta.innerHTML=`${c(t.track,!0)}<span class="stage-slash">/</span><span>${t.label}</span>`,d.stageEyebrow.textContent=t.eyebrow,d.stageTitle.textContent=t.title,d.stageDescription.textContent=t.description,d.previous.disabled=f===0,d.next.setAttribute(`aria-label`,f===i.length-1?`回到开场`:`下一章节`),x(t,e),y(),S(),C()}function T(e,t=!1){m=!0,p=t,f=Math.max(0,Math.min(i.length-1,e)),d.showcase.classList.add(`is-started`),w()}function E(){m=!0,p=!0,f=0,d.showcase.classList.add(`is-started`),w()}function D(){m=!0,p=!p,d.showcase.classList.add(`is-started`),S(),C()}document.querySelector(`#startShow`).addEventListener(`click`,E),document.querySelector(`#openCurrent`).addEventListener(`click`,()=>{window.open(i[f].url,`_blank`,`noopener,noreferrer`)}),document.querySelector(`#enterFullscreen`).addEventListener(`click`,()=>{document.documentElement.requestFullscreen?.()}),d.previous.addEventListener(`click`,()=>T(f-1,!1)),d.next.addEventListener(`click`,()=>{T(f===i.length-1?0:f+1,!1)}),d.toggle.addEventListener(`click`,D),window.addEventListener(`keydown`,e=>{!m||e.target instanceof Element&&e.target.closest(`button, a, input, textarea, iframe`)||(e.key===`ArrowLeft`&&T(f-1,!1),e.key===`ArrowRight`&&T(f+1,!1),e.code===`Space`&&(e.preventDefault(),D()),e.key.toLowerCase()===`f`&&document.documentElement.requestFullscreen?.())}),w({posterOnly:!0});